...
public class MediaController extends FrameLayout {
  ...
  private ImageButton mFullOrExitButton;   //屏幕缩放按钮
  private boolean isFullScreen;    //用于判断屏幕是否是全屏
  private Activity activity;               //用于控制屏幕的方向
   ...
   *在此设置屏幕的缩放
   */
  private View.OnClickListener mFullOrExitListener = new View.OnClickListener() {
    public void onClick(View v) {
      if (isFullScreen == false) {   //如果屏幕不是全屏，则设置成横向全屏
        isFullScreen = true;
       // Toast.makeText(getContext(),"set full screen",Toast.LENGTH_SHORT).show();
        mFullOrExitButton.setImageResource(getResources().getIdentifier(
                "fullscreen_exit", "drawable", mContext.getPackageName()));   //改变图标
        activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE); //设置成横屏
      }
      else {     //如果屏幕是全屏，则还原原来的尺寸
        isFullScreen = false;
       // Toast.makeText(getContext(),"set exit screen", Toast.LENGTH_SHORT).show();
        mFullOrExitButton.setImageResource(getResources().getIdentifier(
                "fullscreen_alt", "drawable", mContext.getPackageName()));    //改变图标

        activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT); //设置成竖屏
  ...
  private void initControllerView(View v) {
    ...
    /*初始化屏幕缩放按钮*/
    mFullOrExitButton = (ImageButton) v.findViewById(getResources().getIdentifier("full_exit_button", "id", mContext.getPackageName()));
    if (mFullOrExitButton != null) {
      mFullOrExitButton.requestFocus();
      mFullOrExitButton.setOnClickListener(mFullOrExitListener);
    }
